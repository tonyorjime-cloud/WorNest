...
def _fix_attachment_id_sequences():
    try:
        ...
        try:
            execute("ALTER TABLE ncrs ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY")
        except Exception:
            pass
...
from core.projects_service import (
    ...
    _module_summary_lines,
    _structured_module_markdown,
...
"""
    ...
    try:
        if DB_IS_POSTGRES:
            ...
            execute("UPDATE leaves SET status='APPROVED' WHERE UPPER(COALESCE(status,''))='PENDING'")
            ...
            try: execute("UPDATE leaves SET status='APPROVED' WHERE UPPER(COALESCE(status,''))='PENDING'")
            except Exception: pass
    ...
    try:
        ...
        execute("""CREATE TABLE IF NOT EXISTS ncrs (
            id INTEGER PRIMARY KEY,
            project_id INTEGER NOT NULL,
            report_id INTEGER,
            created_by INTEGER,
        ...
        execute("""CREATE TABLE IF NOT EXISTS ncr_attachments (
            id INTEGER PRIMARY KEY,
        ...
        extra_cols = [
            ...
            "ALTER TABLE test_results ADD COLUMN notes TEXT",
            "ALTER TABLE test_results ADD COLUMN updated_at TEXT",
            "ALTER TABLE ncrs ADD COLUMN closed_by INTEGER",
            "ALTER TABLE ncrs ADD COLUMN closed_at TEXT",
            "ALTER TABLE ncrs ADD COLUMN closure_remarks TEXT",
            "ALTER TABLE ncrs ADD COLUMN disposition TEXT",
            "ALTER TABLE ncr_attachments ADD COLUMN caption TEXT",
        ]
        for q in extra_cols:
            try:
                execute(q)
...
def main():
    ...
    if "nav_page" in st.session_state:
        ...
        st.session_state["_pending_nav"] = page
        nav_project_id = st.session_state.pop("nav_project_id", None)
        if nav_project_id is not None:
            st.session_state["selected_project_id"] = int(nav_project_id)
